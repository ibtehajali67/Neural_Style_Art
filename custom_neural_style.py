# commend for running the code
'''
python neural_style.py --content_img src_img1.png --style_imgs tar_img3.png --max_size 512 --max_iterations 500 --original_colors --device /gpu:0 --verbose;
'''
...
def parse_args():
  ...
  parser.add_argument('--original_colors', action='store_false',default=False,
  ...
  # Add argument for CUSTOM NFTs
  parser.add_argument('--content_img_dir', type=str,
    default='./',
    help='Directory path to the content_img_dir images. (default: %(default)s)')
  ...
  parser.add_argument('--output_dir', type=str,
    default='./',
    help='Directory path to the output_dir images. (default: %(default)s)')
...
'''
def stylize(content_img, style_imgs, init_img,Imgval,cnt_val,output_dir, frame=None,):
  with tf.device(args.device), tf.Session() as sess:
    ...
    if args.video:
      ...
      write_image_output(output_img, content_img, style_imgs, init_img,Imgval,cnt_val,output_dir)
...
def write_image_output(output_img, content_img, style_imgs, init_img, Imgval,cnt_val,output_dir):
  out_dir = output_dir
  maybe_make_directory(out_dir)
  img_path = os.path.join(out_dir, f'{Imgval}_{cnt_val}'+'.png')
...
def get_content_image(content_img,content_img_dir):
...
# this code is for one style image on all content images automatically  
def get_style_images(content_img, style_path):
  _, ch, cw, cd = content_img.shape
  style_imgs = []
  # for style_fn in args.style_imgs:
  #   path = os.path.join(args.style_imgs_dir, style_fn)
...
# # this code is for one style image on all content images automatically 
# def render_single_image():
#   content_img_dir = os.listdir('./NFTs/Input1/')
#   style_img_dir = os.listdir('./NFTs/style1/')
  
#   for i in style_img_dir:
#     Imgval = i.split('.')[0]  # name of style imagefile for ex : 256 (256.png)
#     i = os.path.join('./NFTs/style1/',i)  # path of image : ./NFTs/style1/256.png 
#     for j in content_img_dir:
#       cnt_val = j.split('.')[0] # name of content imagefile for ex : 200 (200.png)
#       j = os.path.join('./NFTs/Input1/',j)   # path of image : ./NFTs/style1/200.png      

#       content_img = get_content_image(j)
#       style_imgs = get_style_images(content_img,i) # list contain single style image
#       with tf.Graph().as_default():
#         print('\n---- RENDERING SINGLE IMAGE ----\n')
#         init_img = get_init_image(args.init_img_type, content_img, style_imgs)
#         tick = time.time()
#         stylize(content_img, style_imgs, init_img,Imgval,cnt_val)
#         tock = time.time()
#         print('Single image elapsed time: {}'.format(tock - tick))
    


# Testing purpose for utility : test each content image on all style images.
def render_single_image(content_img_dir,style_img_dir,output_dir):

  content_img_list = os.listdir(content_img_dir)
  style_img_list = os.listdir(style_img_dir)

  # create a function for test each content image apply on all styles images.
  for i in content_img_list:
    Imgval = i.split('.')[0]  # name of style imagefile for ex : 256 (256.png)
    # i = os.path.join(content_img_dir,i)  # path of image : ./NFTs/style1/256.png 
    for j in style_img_list:
      cnt_val = j.split('.')[0] # name of content imagefile for ex : 200 (200.png)
      j = os.path.join(style_img_dir,j)   # path of image : ./NFTs/style1/200.png      

      content_img = get_content_image(i,content_img_dir)
      style_imgs = get_style_images(content_img,j) # list contain single style image
      with tf.Graph().as_default():
        print('\n---- RENDERING SINGLE IMAGE ----\n')
        init_img = get_init_image(args.init_img_type, content_img, style_imgs)
        tick = time.time()
  ...
  # # create a function for test one style apply on one content images.
  # for i, j in zip(content_img_list, style_img_list):
  #   Imgval = i.split('.')[0]  
  #   cnt_val = j.split('.')[0] 

  #   j = os.path.join(style_img_dir,j)  # it contain style image path 
  #   content_img = get_content_image(i,content_img_dir) # list contain single content image
  #   style_imgs = get_style_images(content_img,j) # list contain single style image
...
def main():
  ...
  style_img_dir = args.style_img_dir
